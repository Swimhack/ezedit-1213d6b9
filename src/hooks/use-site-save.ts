...
export function useSiteSave() {
  ...
  const saveSite = async (
    ...
    console.log("[useSiteSave] Starting saveSite with formData:", formData); 
    console.log("[useSiteSave] Existing site:", existingSite);
    ...
    try {
      ...
      console.log("[useSiteSave] Session data:", sessionData);
      
      if (sessionError) {
        console.error("[useSiteSave] Session error:", sessionError);
        throw new Error(`Authentication error: ${sessionError.message}`);
      }

      if (!sessionData?.session?.user) {
        console.error("[useSiteSave] No user in session");
        toast.error("You must be logged in to save sites");
        return false;
      }

      const userId = sessionData.session.user.id;
      console.log("[useSiteSave] User ID for save operation:", userId);
      ...
      console.log("[useSiteSave] Prepared data for Supabase:", preparedData);

      let result;
      
      // Prepare the data object based on whether it's a new site or an update
      if (existingSite) {
        console.log("[useSiteSave] Updating existing site with ID:", existingSite.id);
        ...
        console.log("[useSiteSave] Inserting new site");
      ...
      console.log("[useSiteSave] Supabase operation complete. Result:", result);
      
      if (result.error) {
        console.error("[useSiteSave] Supabase error on save:", result.error);
        throw new Error(`Database error: ${result.error.message}`);
      }

      console.log("[useSiteSave] Site saved successfully");
      return true;
    } catch (error: any) {
      console.error("[useSiteSave] Error in saveSite function:", error);
